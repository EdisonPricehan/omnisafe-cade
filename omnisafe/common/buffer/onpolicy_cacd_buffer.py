...
class OnPolicyCACDBuffer(BaseBuffer):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        self.ep_ret_swf: SlidingWindowFilter = SlidingWindowFilter(ep_ret_window_size)
        self.mean_ep_ret: float = 0  # mean episode return (cumsum of rewards of a single trajectory)
        self.max_ep_ret: float = 0  # maximum episode return (cumsum of rewards of a single trajectory)

        assert self._penalty_coefficient >= 0, 'penalty_coefficient must be non-negative!'
        assert self._advantage_estimator in ['gae', 'gae-rtg', 'vtrace', 'plain', 'subm']
