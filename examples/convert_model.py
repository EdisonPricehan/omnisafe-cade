...
from omnisafe.models.actor_critic.cade_wrapper import CadeWrapper
from omnisafe.utils.config import Config


def dummy_input() -> Tuple[torch.Tensor, torch.Tensor]:
    dummy_obs = torch.zeros((1, *obs_space.shape), dtype=torch.float32)
    dummy_reset = torch.tensor([[0]], dtype=torch.bool)
    return dummy_obs, dummy_reset
...
def load_model_wrapper(model_path: str) -> CadeWrapper:
    assert os.path.exists(model_path), f'Model path {model_path} does not exist!'

    cade_wrapper = CadeWrapper(
        obs_space=obs_space,
        act_space=act_space,
        model_cfgs=cfgs.model_cfgs,
...
def pth2onnx(pth_path: str, onnx_name: str = 'model.onnx') -> None:
    ...
    torch.onnx.export(
        model,
        (dummy_obs, dummy_reset),
        target_onnx_path,
        export_params=True,
        opset_version=11,
        do_constant_folding=True,
        input_names=['obs', 'reset_flag'],
        output_names=['action'],
        # dynamic_axes={
        #     'obs': {0: 'batch_size'},
        #     'reset_flag': {0: 'batch_size'},
        #     'action': {0: 'batch_size'}
        # }
    )
...
if __name__ == '__main__':
    ...
    cade_onnx_name: str = 'policy.onnx'
